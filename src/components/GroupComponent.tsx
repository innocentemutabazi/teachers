import { FunctionComponent } from "react";
import { css } from "@emotion/css";

export type GroupComponentType = {
  className?: string;
  courseCardBackgrounds?: string;
  webDesignBasicToAdvance?: string;
  group119?: string;
};

const GroupComponent: FunctionComponent<GroupComponentType> = ({
  className = "",
  courseCardBackgrounds,
  webDesignBasicToAdvance,
  group119,
}) => {
  return (
    <div
      className={[
        css`
          width: 419px;
          box-shadow: 0px 0px 20px rgba(0, 0, 0, 0.1);
          border-radius: var(--br-6xl);
          background-color: var(--pure-white);
          display: flex;
          flex-direction: column;
          align-items: flex-end;
          justify-content: flex-start;
          padding: 0px 0px var(--padding-8xl);
          box-sizing: border-box;
          gap: var(--gap-16xl);
          min-width: 398;
          max-width: 100%;
          height: auto;
          text-align: left;
          font-size: var(--font-size-6xl);
          color: var(--color-gray-300);
          font-family: var(--font-roboto-slab);
          &:hover {
            background-color: var(--pure-white);
            display: flex;
            width: 419px;
            height: auto;
            flex-direction: column;
            gap: var(--gap-16xl);
            align-items: flex-end;
            justify-content: flex-start;
            border-radius: var(--br-6xl);
            padding: 0px 0px var(--padding-8xl);
            box-sizing: border-box;
            box-shadow: 0px 0px 20px rgba(0, 0, 0, 0.1);
            min-width: 398;
            max-width: 100%;
          }
          @media screen and (max-width: 450px) {
            gap: var(--gap-mid);
            min-width: 100%;
          }
        `,
        className,
      ].join(" ")}
    >
      <img
        className={css`
          align-self: stretch;
          height: 250px;
          position: relative;
          border-radius: var(--br-6xl) var(--br-6xl) 0px 0px;
          max-width: 100%;
          overflow: hidden;
          flex-shrink: 0;
          object-fit: cover;
          background-color: var(--pure-white);
          display: flex;
          width: auto;
          z-index: 1;
          &:hover {
            background-color: var(--pure-white);
            display: flex;
            align-self: stretch;
            width: auto;
            height: 250px;
            border-radius: var(--br-6xl) var(--br-6xl) 0px 0px;
          }
        `}
        loading="lazy"
        alt=""
        src={courseCardBackgrounds}
      />
      <div
        className={css`
          display: flex;
          flex-direction: row;
          align-items: flex-start;
          justify-content: flex-end;
          padding: 0px var(--padding-2xl);
          box-sizing: border-box;
          max-width: 100%;
          width: auto;
          align-self: unset;
          height: auto;
          gap: var(--gap-0);
          transform: rotate(0deg);
          &:hover {
            display: flex;
            width: auto;
            align-self: unset;
            height: auto;
            flex-direction: row;
            gap: var(--gap-0);
            align-items: flex-start;
            justify-content: flex-end;
            transform: rotate(0deg);
            padding: 0px var(--padding-2xl);
            box-sizing: border-box;
            max-width: 100%;
          }
        `}
      >
        <div
          className={css`
            display: flex;
            flex-direction: column;
            align-items: flex-start;
            justify-content: flex-start;
            gap: var(--gap-20xl-5);
            max-width: 100%;
            width: auto;
            align-self: unset;
            height: auto;
            transform: rotate(0deg);
            &:hover {
              display: flex;
              width: auto;
              align-self: unset;
              height: auto;
              flex-direction: column;
              gap: var(--gap-20xl-5);
              align-items: flex-start;
              justify-content: flex-start;
              transform: rotate(0deg);
              max-width: 100%;
            }
            @media screen and (max-width: 450px) {
              gap: var(--gap-xl);
            }
          `}
        >
          <div
            className={css`
              display: flex;
              flex-direction: row;
              align-items: flex-start;
              justify-content: flex-start;
              padding: 0px 0px 0px var(--padding-6xs);
              box-sizing: border-box;
              width: auto;
              align-self: unset;
              height: auto;
              gap: var(--gap-0);
              transform: rotate(0deg);
              &:hover {
                display: flex;
                width: auto;
                align-self: unset;
                height: auto;
                flex-direction: row;
                gap: var(--gap-0);
                align-items: flex-start;
                justify-content: flex-start;
                transform: rotate(0deg);
                padding: 0px 0px 0px var(--padding-6xs);
                box-sizing: border-box;
              }
            `}
          >
            <b
              className={css`
                position: relative;
                line-height: 125%;
                display: flex;
                white-space: pre-wrap;
                font-weight: 700;
                width: auto;
                align-self: unset;
                height: auto;
                z-index: 1;
                &:hover {
                  font-weight: 700;
                  font-family: var(--font-roboto-slab);
                  font-size: var(--font-size-6xl);
                  line-height: 125%;
                  text-align: left;
                  color: var(--color-gray-300);
                  display: flex;
                  width: auto;
                  align-self: unset;
                  height: auto;
                }
                @media screen and (max-width: 450px) {
                  font-size: var(
                    --previewthemeforestnet-kumbh-sans-medium-20-size
                  );
                  line-height: 25px;
                }
              `}
            >
              {webDesignBasicToAdvance}
            </b>
          </div>
          <div
            className={css`
              width: 279px;
              display: flex;
              flex-direction: row;
              align-items: flex-start;
              justify-content: flex-start;
              padding: 0px var(--padding-6xs);
              box-sizing: border-box;
              height: auto;
              gap: var(--gap-0);
              transform: rotate(0deg);
              font-size: var(--font-size-lg);
              color: var(--color-dimgray-100);
              font-family: var(--font-poppins);
              &:hover {
                display: flex;
                width: 279px;
                height: auto;
                flex-direction: row;
                gap: var(--gap-0);
                align-items: flex-start;
                justify-content: flex-start;
                transform: rotate(0deg);
                padding: 0px var(--padding-6xs);
                box-sizing: border-box;
              }
            `}
          >
            <div
              className={css`
                flex: 1;
                display: flex;
                flex-direction: row;
                align-items: flex-end;
                justify-content: flex-start;
                gap: var(--gap-13xl);
                height: auto;
                transform: rotate(0deg);
                &:hover {
                  display: flex;
                  flex: 1;
                  height: auto;
                  flex-direction: row;
                  gap: var(--gap-13xl);
                  align-items: flex-end;
                  justify-content: flex-start;
                  transform: rotate(0deg);
                }
                @media screen and (max-width: 450px) {
                  flex-wrap: wrap;
                  gap: var(--gap-base);
                }
              `}
            >
              <div
                className={css`
                  flex: 1;
                  display: flex;
                  flex-direction: column;
                  align-items: flex-start;
                  justify-content: flex-end;
                  padding: 0px 0px var(--padding-12xs);
                  box-sizing: border-box;
                  min-width: 78;
                  height: auto;
                  gap: var(--gap-0);
                  transform: rotate(0deg);
                  &:hover {
                    display: flex;
                    flex: 1;
                    height: auto;
                    flex-direction: column;
                    gap: var(--gap-0);
                    align-items: flex-start;
                    justify-content: flex-end;
                    transform: rotate(0deg);
                    padding: 0px 0px var(--padding-12xs);
                    box-sizing: border-box;
                    min-width: 78;
                  }
                `}
              >
                <img
                  className={css`
                    align-self: stretch;
                    height: 24px;
                    position: relative;
                    max-width: 100%;
                    overflow: hidden;
                    flex-shrink: 0;
                    display: flex;
                    width: auto;
                    z-index: 1;
                    &:hover {
                      display: flex;
                      align-self: stretch;
                      width: auto;
                      height: 24px;
                    }
                  `}
                  loading="lazy"
                  alt=""
                  src={group119}
                />
              </div>
              <div
                className={css`
                  display: flex;
                  flex-direction: row;
                  align-items: flex-start;
                  justify-content: flex-start;
                  gap: var(--gap-3xs);
                  width: auto;
                  align-self: unset;
                  height: auto;
                  transform: rotate(0deg);
                  &:hover {
                    display: flex;
                    width: auto;
                    align-self: unset;
                    height: auto;
                    flex-direction: row;
                    gap: var(--gap-3xs);
                    align-items: flex-start;
                    justify-content: flex-start;
                    transform: rotate(0deg);
                  }
                `}
              >
                <div
                  className={css`
                    position: relative;
                    line-height: 150%;
                    font-weight: 500;
                    display: flex;
                    min-width: 29;
                    width: auto;
                    align-self: unset;
                    height: auto;
                    z-index: 1;
                    &:hover {
                      font-weight: 500;
                      font-family: var(--font-poppins);
                      font-size: var(--font-size-lg);
                      line-height: 150%;
                      text-align: left;
                      color: var(--color-dimgray-100);
                      display: flex;
                      width: auto;
                      align-self: unset;
                      height: auto;
                      min-width: 29;
                    }
                  `}
                >
                  5.0
                </div>
                <div
                  className={css`
                    display: flex;
                    flex-direction: column;
                    align-items: flex-start;
                    justify-content: flex-start;
                    padding: var(--padding-11xs) 0px 0px;
                    box-sizing: border-box;
                    width: auto;
                    align-self: unset;
                    height: auto;
                    gap: var(--gap-0);
                    transform: rotate(0deg);
                    font-size: var(--font-size-mini);
                    &:hover {
                      display: flex;
                      width: auto;
                      align-self: unset;
                      height: auto;
                      flex-direction: column;
                      gap: var(--gap-0);
                      align-items: flex-start;
                      justify-content: flex-start;
                      transform: rotate(0deg);
                      padding: var(--padding-11xs) 0px 0px;
                      box-sizing: border-box;
                    }
                  `}
                >
                  <div
                    className={css`
                      position: relative;
                      line-height: 23px;
                      font-weight: 500;
                      display: flex;
                      min-width: 74;
                      width: auto;
                      align-self: unset;
                      height: auto;
                      z-index: 1;
                      &:hover {
                        font-weight: 500;
                        font-family: var(--font-poppins);
                        font-size: var(--font-size-mini);
                        line-height: 23px;
                        text-align: left;
                        color: var(--color-dimgray-100);
                        display: flex;
                        width: auto;
                        align-self: unset;
                        height: auto;
                        min-width: 74;
                      }
                    `}
                  >
                    (2 rating)
                  </div>
                </div>
              </div>
            </div>
          </div>
          <button
            className={css`
              cursor: pointer;
              border: 2px solid var(--color-steelblue-300);
              padding: var(--padding-4xs) var(--padding-xl);
              background-color: var(--color-steelblue-400);
              width: 347px;
              border-radius: var(--br-mini) 0px var(--br-mini) var(--br-mini);
              box-sizing: border-box;
              display: flex;
              flex-direction: row;
              align-items: flex-start;
              justify-content: center;
              max-width: 100%;
              height: auto;
              gap: var(--gap-3xs);
              z-index: 1;
              &:hover {
                display: flex;
                width: 347px;
                height: auto;
                flex-direction: row;
                gap: var(--gap-3xs);
                align-items: flex-start;
                justify-content: center;
                border-radius: var(--br-mini) 0px var(--br-mini) var(--br-mini);
                padding: var(--padding-4xs) var(--padding-xl);
                box-sizing: border-box;
                border: 2px solid var(--color-steelblue-100);
                max-width: 100%;
              }
              &:active {
                background-color: var(--color-steelblue-400);
                border: 2px solid var(--color-steelblue-300);
                box-sizing: border-box;
              }
            `}
          >
            <div
              className={css`
                position: relative;
                font-size: var(--font-size-3xl);
                line-height: 150%;
                font-weight: 600;
                font-family: var(--font-poppins);
                color: var(--color-dimgray-100);
                text-align: left;
                display: flex;
                min-width: 128;
                width: auto;
                align-self: unset;
                height: auto;
                &:hover {
                  font-weight: 600;
                  font-family: var(--font-poppins);
                  font-size: var(--font-size-3xl);
                  line-height: 150%;
                  text-align: left;
                  color: var(--color-dimgray-100);
                  display: flex;
                  width: auto;
                  align-self: unset;
                  height: auto;
                  min-width: 128;
                }
                @media screen and (max-width: 450px) {
                  font-size: var(--font-size-lg);
                  line-height: 26px;
                }
              `}
            >
              Add to Cart
            </div>
          </button>
        </div>
      </div>
    </div>
  );
};

export default GroupComponent;
